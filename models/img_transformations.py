...
class DelayEmbedder(TsImgEmbedder):
    ...
    def ts_to_img(self, signal, pad=True, mask=0):
        ...
        if len(signal.shape) == 2:
            # If input is 2D (batch, length), add feature dimension
            batch, length = signal.shape
            features = 1
            signal = signal.unsqueeze(-1)  # Add feature dimension: (batch, length, 1)
        else:
    ...
    def img_to_ts(self, img):
        ...
        for i in range(cols - 1):
            ...
            print(f"DEBUG: i={i}, start={start}, end={end}, self.embedding={self.embedding}")
            print(f"DEBUG: target slice shape: {reconstructed_x_time_series[:, :, start:end].shape}")
            print(f"DEBUG: source tensor shape: {img_non_square[:, :, :, i].shape}")
