...
def parse_args_uncond():
    ...
    if parsed_args.dataset in ['stock', 'sine', 'energy', 'mujoco','network_traffic']:
        parsed_args.input_size = parsed_args.input_channels
        # Ensure input_size is at least 1
        if parsed_args.input_size <= 0:
            parsed_args.input_size = 1
            print(f"Warning: input_size was <= 0, setting to 1")
    return parsed_args
