...
def real_data_loading(data_name, seq_len):
    ...
    """
    assert data_name in ['stock', 'energy', 'metro', 'network_traffic']

    if data_name == 'stock':
        ori_data = np.loadtxt('data/short_range/stock_data.csv', delimiter=",", skiprows=1)
    if data_name == 'network_traffic':

        # Try packet sizes file first (separated for training)
        ori_data = np.loadtxt('data/ultra_long_range/youtube_traffic_bytes_only.csv', delimiter=",", skiprows=1)
        print("Loaded packet sizes only (separated data)")

        # For single feature, reshape to add feature dimension
        if len(ori_data.shape) == 1:
            ori_data = ori_data.reshape(-1, 1)  # Convert to (length, 1)
...
def gen_dataloader(args):
    ...
    elif args.dataset in ['stock', 'energy', 'network_traffic']:
