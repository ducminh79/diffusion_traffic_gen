...
def PCA_plot(prep_ori, prep_gen, anal_sample_no, logger, args, save_path=None):
    ...
    if save_path:
        plt.savefig(f"{save_path}/{args.dataset}_pca_plot.png", dpi=300, bbox_inches='tight')
        plt.savefig(f"{save_path}/{args.dataset}_pca_plot.pdf", bbox_inches='tight')
...
def TSNE_plot(prep_ori, prep_gen, anal_sample_no, logger, args, save_path=None):
    colors = ["red" for i in range(anal_sample_no)] + ["blue" for i in range(anal_sample_no)]
    ...
    # Save the figure if path is provided
    if save_path:
        plt.savefig(f"{save_path}/{args.dataset}_tsne_plot.png", dpi=300, bbox_inches='tight')
        plt.savefig(f"{save_path}/{args.dataset}_tsne_plot.pdf", bbox_inches='tight')
...
def density_plot(prep_ori, prep_gen, logger, args, save_path=None):
    f, ax = plt.subplots(1)
    ...
    # Save the figure if path is provided
    if save_path:
        plt.savefig(f"{save_path}/{args.dataset}_density_plot.png", dpi=300, bbox_inches='tight')
        plt.savefig(f"{save_path}/{args.dataset}_density_plot.pdf", bbox_inches='tight')
